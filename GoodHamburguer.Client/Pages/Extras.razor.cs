...
public partial class Extras
{
    ...
    private Guid? ExtrasCategoryId { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var extrasCategory = categories.FirstOrDefault(c => c.Name.Equals("Extras", StringComparison.OrdinalIgnoreCase));
        if (extrasCategory != null)
        {
            ExtrasCategoryId = extrasCategory.Id;
        }
    ...
    private async Task LoadExtrasAsync()
    {
        try
        {
            ...
            if (ExtrasCategoryId == null)
            {
                ErrorMessage = "Extras category not found.";
                ProductsList = new PagedResultDto<ProductDto>(Enumerable.Empty<ProductDto>(), 1, PageSize, 0, 0);
                return;
            }

            ProductsList = await ProductService.GetProductsByCategoryAsync(ExtrasCategoryId.Value, CurrentPage, PageSize);
        }
        catch (Exception ex)
        {
            ErrorMessage = $"Error loading extras: {ex.Message}";
            ProductsList = new PagedResultDto<ProductDto>(Enumerable.Empty<ProductDto>(), 1, PageSize, 0, 0);
        }
        finally
        {
